...
trait PopupUpdate extends Audio {
  ...
  println(i,j)
    ...
    verticalSubMenu2.action = Action("Vertical"){
      if(!player.placeShip(i,j,ShipType.BATTLESHIP,ShipOrientation.VERTICAL))playError()
      caller.update()
    }
    verticalSubMenu3.action = Action("Vertical"){
      if(!player.placeShip(i,j,ShipType.DESTROYER,ShipOrientation.VERTICAL))playError()
      caller.update()
    }
    verticalSubMenu4.action = Action("Vertical"){
      if(!player.placeShip(i,j,ShipType.SUBMARINE,ShipOrientation.VERTICAL))playError()
      caller.update()
    }
    verticalSubMenu5.action = Action("Vertical"){
      if(!player.placeShip(i,j,ShipType.CARRIER,ShipOrientation.VERTICAL))playError()
      caller.update()
    }

    //create function for the horizontal calls
    horizontalSubMenu1.action =  Action("Horizontal"){
      if(!player.placeShip(i,j,ShipType.PATROL,ShipOrientation.HORIZONTAL))playError()
      caller.update()
    }
    horizontalSubMenu2.action = Action("Horizontal"){
      if(!player.placeShip(i,j,ShipType.BATTLESHIP,ShipOrientation.HORIZONTAL))playError()
      caller.update()
    }
    horizontalSubMenu3.action = Action("Horizontal"){
      if(!player.placeShip(i,j,ShipType.DESTROYER,ShipOrientation.HORIZONTAL))playError()
      caller.update()
    }
    horizontalSubMenu4.action = Action("Horizontal"){
      if(!player.placeShip(i,j,ShipType.SUBMARINE,ShipOrientation.HORIZONTAL))playError()
      caller.update()
    }
    horizontalSubMenu5.action = Action("Horizontal"){
      if(!player.placeShip(i,j,ShipType.CARRIER,ShipOrientation.HORIZONTAL))playError()
      caller.update()
    }
